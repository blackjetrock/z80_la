...
void print_capture_buf2(const uint32_t *buf, uint pin_base, uint pin_count, uint32_t n_samples) {
...
void print_capture_buf(const uint32_t *buf, uint pin_base, uint pin_count, uint32_t n_samples) {
  ...
  for (int sample = 0; sample < n_samples; ++sample) {
    ...
    // Data is left-justified in each FIFO entry, hence the (32 - record_size_bits) offset
    //uint word_mask = 1u << (bit_index % record_size_bits + 32 - record_size_bits);
    int gpios = buf[sample];
    int data = gpios & 0xff;
    int addr = (gpios >> 8) & 0x7FFF;
    addr |= ((gpios & (1<<26)) >> 26) << 15;
    int rd = (gpios & (1<<28))>>28;
    
    printf("%06X: %08X %04X %02X %d\n", sample, gpios, addr, data, rd);
  }
  printf("\n");
}
...
int main() {
    ...
    logic_analyser_init(pio, sm, CAPTURE_PIN_BASE, CAPTURE_PIN_COUNT, CLOCK_DIV);

    printf("Arming trigger\n");
    //    logic_analyser_arm(pio, sm, dma_chan, capture_buf, buf_size_words, CAPTURE_PIN_BASE, true);

    
    // Trigger off reset going high
    logic_analyser_arm(pio, sm, dma_chan, capture_buf, buf_size_words, 27, true);
